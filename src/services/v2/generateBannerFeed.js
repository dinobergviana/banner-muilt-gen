...
import { CORDINATES_FEED } from "../../consts/cordinates.js";
...
export const generateBannerImageFeed = async (data, path) => {
  ...
  const titleCordinateX = CORDINATES_FEED[daysKeys[dayIndex]].TITLE.EIXO_X;
  const titleCordinateY = CORDINATES_FEED[daysKeys[dayIndex]].TITLE.EIXO_Y;

  const hourCordinateX = CORDINATES_FEED[daysKeys[dayIndex]].HOUR.EIXO_X;
  const hourCordinateY = CORDINATES_FEED[daysKeys[dayIndex]].HOUR.EIXO_Y;

  const districtCordinateX =
    CORDINATES_FEED[daysKeys[dayIndex]].DISTRICT.EIXO_X;
  const districtCordinateY =
    CORDINATES_FEED[daysKeys[dayIndex]].DISTRICT.EIXO_Y;

  let localCordinateX = 470;
  let localCordinateY = 925;

  let addressCordinateX = 570;
  let addressCordinateY = 925;
  ...
  let basePosition = 470;

  let lidershipCordinateX =
    basePosition - banner.district.length - banner.lidership.length * 10;
  let lidershipCordinateY = 970;

  let phoneCordinateX = 780;
  let phoneCordinateY = 970;
  ...
  templatePath = `./src/banners/template-v2/${daysKeys[dayIndex]} - FEED.jpg`;
  ...
  function addGCName(gcName) {
    ...
    context.fillStyle = CORDINATES_FEED[daysKeys[dayIndex]].TITLE.COLOR;
